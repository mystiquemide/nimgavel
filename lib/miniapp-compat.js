...
async function recoverEmbeddedWallet() {
  if (!isEmbeddedNimiqPay() || !routeNeedsWallet() || walletProbeRunning || walletProbeFinished) return;

  const status = getBootState().status;
  if (!["idle", "spectate"].includes(status)) {
    if (status === "ready" || status === "timeout" || status === "cancelled" || status === "no_accounts" || status === "paddle_error") {
  ...
  try {
    // Nimiq Pay versions can expose either the app bridge or the provider
    // first. If either exists, recover from an early spectator classification
    // instead of leaving the page permanently disconnected.
    if (status === "spectate") resetBoot();
    const result = await bootWallet();
    if (result.walletState === "ready") {
      walletProbeFinished = true;
      // Re-render the active floor/host/room surface so it picks up the
      // newly-ready wallet and paddle session.
      window.dispatchEvent(new PopStateEvent("popstate"));
    } else if (result.walletState !== "spectate") {
      // bootWallet already performs the one safe timeout retry. Do not
      // automatically re-prompt after that, cancellation, no accounts, or
...
function enforcePhotoHiddenState() {
  if (!document.getElementById("nimgavel-photo-hidden-fix")) {
    const style = document.createElement("style");
    style.id = "nimgavel-photo-hidden-fix";
    style.textContent = ".photo-selected[hidden]{display:none!important}";
...
document.addEventListener("click", (event) => {
  ...
  if (!link || isEmbeddedNimiqPay() || isMobileDevice()) return;
  event.preventDefault();
  void showDesktopPayHandoff(link.getAttribute("href"));
});

// Clear stale photo preview data after the host form's own remove handler runs.
document.addEventListener("click", (event) => {
  const target = event.target instanceof Element ? event.target.closest("#btn-remove-photo") : null;
  if (!target) return;
  queueMicrotask(() => {
    const selected = document.querySelector("#photo-selected");
    const thumb = document.querySelector("#photo-selected-thumb");
