...
export function bootWallet() {
  ...
  bootPromise = (async () => {
    ...
    try {
      ...
      // A slow first launch inside Nimiq Pay can miss the provider/account
      // response even though the app bridge is already present. Retry once
      // only for that timeout. Never silently retry a user rejection.
      if (walletState === "timeout" && typeof window !== "undefined" && window.nimiqPay) {
        await wait(500);
