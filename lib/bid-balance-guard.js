...
WebSocket.prototype.send = function nimgavelBalanceBackedSend(data) {
  ...
  const cachedProof = proofs.get(lotId);
  if (!cachedProof && pendingProofs.has(lotId)) {
    showBidNotice("Confirm your bidder wallet first. Your original bid is waiting.");
    return undefined;
  }
...
function ensureBalanceNotice() {
  ...
  note.textContent = "Balance checked at bid time. Funds stay in your wallet until settlement.";
