...
//   | validityStartHeight(4) | networkId(1) | flags(1)=0x00 | Albatross sender_data length(1)=0x00
...
export function deriveTxHash(serializedTx) {
  ...
  const albatross = [5, 6, 7, 24].includes(tx.networkId);
  const content = new Uint8Array(albatross ? 67 : 66);
  ...
  offset += 1;
  if (albatross) content[offset++] = 0x00;
