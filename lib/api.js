...
export const createHostChallenge = (hostAddress, lotId) =>
  request("/api/host/challenge", { method: "POST", body: { hostAddress, ...(lotId ? { lotId } : {}) } });

export const authorizeLotHost = (lotId, proof) =>
  request(`/api/lots/${encodeURIComponent(lotId)}/authorize`, { method: "POST", body: proof });

export const removeBid = (lotId, bidId, { role, token, reason }) =>
  request(`/api/lots/${encodeURIComponent(lotId)}/bids/${encodeURIComponent(bidId)}/remove`, {
    method: "POST", headers: { authorization: `Bearer ${token}` }, body: { role, reason }
  });
