...
if (typeof window !== "undefined" && import.meta.env.DEV) {
  ...
  if (params.get("reviewWallet") === "1" && !wallet.account) {
    ...
    wallet.accounts = [wallet.account];
  }
}
...
function getInjectedProvider() {
  ...
  if (window.nimiqPay && typeof window.nimiqPay.listAccounts === "function") return window.nimiqPay;
  return null;
}
...
export async function connectWallet({ providerTimeoutMs = 15000, accountTimeoutMs = 45000 } = {}) {
  ...
  // Prefer a provider that Nimiq Pay has already injected. Waiting for init()
  // first can add a full timeout to a Mini App that is actually ready already.
  ...
  if (!provider) {
    try {
      ...
      // The bridge can appear while init() is waiting. Check once more before
      // classifying the page as spectator/provider-timeout.
  ...
  // call could create duplicate native permission prompts on slower devices.
  let accounts;
  try {
  ...
  const sharedAccounts = Array.isArray(accounts)
    ? [...new Set(accounts.filter((address) => typeof address === "string" && address.trim()).map((address) => address.trim()))]
    : [];
  ...
  // Preserve the full wallet-approved address set. Some Nimiq Pay users have
  // multiple NIM addresses, and the message-signing key is not a safe proxy
  // for which shared address actually holds the funds used for settlement.
  wallet.accounts = sharedAccounts;
  wallet.account = sharedAccounts[0];
  wallet.state = "ready";
  return wallet.state;
}
...
export async function signMessage(message) {
  ...
  try {
    result = await withTimeout(
      provider.sign(String(message)),
      60000,
      "The signature request timed out. Check Nimiq Pay for an open confirmation, then try again.",
      WalletTimeoutError
    );
  } catch (error) {
    // Nimiq Pay versions can reject the provider promise directly when the
    // native confirmation sheet is dismissed. Normalize that just like the
    // payment path so callers do not mistake a wallet rejection for an app
    // or API failure.
    if (error instanceof WalletTimeoutError) throw error;
    if (isWalletCancellation(error)) {
      throw new WalletCancelledError(error?.message || "The signing request was cancelled.");
    }
    if (isUnavailableError(error)) {
      throw new Error("Nimiq Pay's signing bridge is unavailable. Retry the wallet connection and try again.");
    }
    throw new Error(error?.message || "Nimiq Pay could not complete the signature request.");
  }
  ...
  if (isErrorResponse(result)) {
    ...
    throw new Error(result.error?.message || "Nimiq Pay could not complete the signature request.");
  }
