...
// States: connecting -> ready | spectate | timeout | cancelled | no_accounts.
...
const wallet = {
  ...
  state: typeof window !== "undefined" && (window.nimiq || window.nimiqPay) ? "connecting" : "spectate",
  provider: null,
  account: null
};
...
export async function connectWallet({ timeoutMs = 15000 } = {}) {
  ...
  try {
    ...
    // A plain browser is spectator mode. If the Nimiq Pay bridge is already
    // present, however, provider injection is simply late and should be
    // treated as a recoverable timeout rather than a user cancellation.
    const embedded = typeof window !== "undefined" && Boolean(window.nimiqPay || window.nimiq);
    wallet.provider = null;
    wallet.account = null;
    wallet.state = embedded ? "timeout" : "spectate";
    return wallet.state;
  }

  // The provider can be present before its permission/account response is
  // ready. Keep this bounded, but distinguish a slow response from a real
  // wallet rejection so the session layer can safely retry only timeouts.
  let accounts;
  try {
    ...
    wallet.account = null;
    wallet.state = error instanceof WalletTimeoutError ? "timeout" : "cancelled";
    return wallet.state;
  }
...
function isWalletCancellation(error) {
  ...
  if (type === "PermissionDeniedError" || type === "AbortError") return true;
