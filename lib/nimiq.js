...
export async function connectWallet({ providerTimeoutMs = 15000, accountTimeoutMs = 45000 } = {}) {
  ...
  try {
    ...
    // Some Nimiq Pay WebViews expose window.nimiq before init() resolves.
    // If the actual injected provider is already usable, fall back to it
    // instead of forcing a refresh or showing a false connection failure.
    const injected = typeof window !== "undefined"
      ? (window.nimiq && typeof window.nimiq.listAccounts === "function"
          ? window.nimiq
          : window.nimiqPay && typeof window.nimiqPay.listAccounts === "function"
            ? window.nimiqPay
            : null)
      : null;
    if (injected) {
